...
export function WhisperTranscription() {
	...
	// Function to stop recording and processing
	const stopRecordingAndProcessing = () => {
		if (recorderRef.current && recording) {
			recorderRef.current.stop();
			setRecording(false);
			setIsPaused(true);

			// Signal worker to stop processing if it's currently processing
			if (isProcessing && worker.current) {
	...
	const resumeRecording = () => {
		if (recorderRef.current && !recording && isPaused) {
	...
	}, [status, recording, isProcessing, chunks, language, isPaused]);

	if (!isWebGPUAvailable) {
		// no fallback mechanizm to wasm.
		return (
	...
	return (
		<div className="flex flex-col h-screen mx-auto justify-end text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-900">
			<div className="h-full mt-10 overflow-auto scrollbar-thin flex justify-start items-center flex-col relative">
				...
				<div className="flex flex-col items-center px-4">
					{micPermissionError && (
						<div className="bg-destructive/10 border border-destructive rounded-lg p-2 mb-3 max-w-[500px]">
							<p className="text-destructive font-medium">Microphone permission denied</p>
							<Button
								className="mt-2 w-full bg-destructive hover:bg-destructive/90 text-white"
					...
					{status === null && (
						<>
							<p className="max-w-[480px] mb-8 text-muted-foreground text-pretty text-wrap-balance">
								<br />
								You are about to load whisper-base, a 73 million parameter speech recognition model.
								Once downloaded, the model (~200 MB) will be cached and reused when you revisit the
								page.
								<br />
								<br />
              ...
              <p className="text-sm text-muted-foreground">Audio Waveform:</p>
						...
						<div className="bg-background dark:bg-background rounded-lg p-3 flex items-center justify-center h-[80px] overflow-hidden shadow-inner border border-gray-800">
						...
						{status === 'ready' && (
							<div className="relative mt-4">
								...
								{tps && (
									<span className="absolute bottom-0 right-0 px-1 text-xs text-primary">
										{tps.toFixed(2)} tok/s
									</span>
								)}
							</div>
					...
					{status === 'ready' && (
						<div className="relative w-full flex justify-center mt-2 gap-2">
							<LanguageSelector
								language={language}
								setLanguage={(e) => {
									recorderRef.current?.stop();
							...
							<div className="flex gap-2 absolute right-2">
								{recording ? (
									<Button
										...
										onClick={stopRecordingAndProcessing}
									>
										Stop
									</Button>
								) : isPaused ? (
								...
								<button
									className="border rounded-lg px-2 hover:bg-primary/10 transition-colors"
									onClick={() => {
										recorderRef.current?.stop();
										recorderRef.current?.start();
									}}
