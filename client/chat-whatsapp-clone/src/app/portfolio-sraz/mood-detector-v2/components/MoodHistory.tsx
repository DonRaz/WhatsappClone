...
import { MoodHistoryProps, MoodType } from '../types';

interface MoodIconConfig {
  icon: React.ComponentType<{ className?: string }>;
  color: string;
  bg: string;
}

const moodIcons: Record<MoodType, MoodIconConfig> = {
...
export default function MoodHistory({ moodDetections }: MoodHistoryProps) {
  const detections = moodDetections || [];
  return (
        ...
        <div className="space-y-3 max-h-64 md:max-h-80 lg:max-h-96 overflow-y-auto">
          {detections.length === 0 ? (
            ...
            detections.map((detection) => {
              const moodConfig = moodIcons[detection.detected_mood];
              const MoodIcon = moodConfig?.icon || Smile;
              const iconColor = moodConfig?.color || 'text-gray-500';
              const bgColor = moodConfig?.bg || 'bg-gray-50';
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-gray-600">
                          {detection.created_date ? format(new Date(detection.created_date), 'h:mm a') : 'Just now'}
                        </p>
