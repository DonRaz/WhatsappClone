...
const processMessage = (messageData: any): MessageData => {
	try {
		...
		const normalizedData = {
			...
			usersSeen: messageData.usersSeen?.map((user: any) => {
		...
		// Parse and validate with Zod schema
		const validatedMessage: MessageData = messageSchema.parse(normalizedData);
...
const processConversation = (conversationData: any): ConversationData => {
	try {
		...
		const normalizedData = {
			...
			isGroup: conversationData.isGroup === null ? false : conversationData.isGroup, // Default to false
			messages: conversationData.messages?.map(processMessage) || [],
			users: conversationData.users?.map(processUser) || [],
		};

		// Parse and validate with Zod schema
		const validatedConversation: ConversationData = conversationSchema.parse(normalizedData);
		...
		return {
			...
			isGroup: conversationData.isGroup === null ? false : !!conversationData.isGroup,
			messages: conversationData.messages?.map(processMessage) || [],
			users: conversationData.users?.map(processUser) || [],
		};
	}
...
export const apiService = {
	...
	async createConversation(data: { userIds: string[]; isGroup?: boolean; name?: string }): Promise<ConversationData> {
	...
	async updateConversation(
		...
		data: { name?: string; addMembers?: string[]; removeMembers?: string[]; imageUrl?: string }
	...
	// Messages
	async sendMessage(conversationId: string, body: string, image?: string): Promise<MessageData> {
	...
	updateUser: async (userData: {
		...
		console.warn('api.updateUser is deprecated, please use updateCurrentUser instead');
		try {
